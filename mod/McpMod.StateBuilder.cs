...
public static partial class McpMod
{
    private static Dictionary<string, object?> BuildGameState()
    {
        ...
        else if (currentRoom is MerchantRoom merchantRoom)
        {
            ...
            else
            {
                ...
                // Report if inventory needs to be opened (no auto-open side effect on GET)
                var merchUI = NMerchantRoom.Instance;
                bool inventoryOpen = merchUI?.Inventory.IsOpen ?? false;
                result["shop"] = BuildShopState(merchantRoom, runState);
                if (!inventoryOpen)
                    ((Dictionary<string, object?>)result["shop"]!)["inventory_closed"] = true;
            }
        }
    ...
    private static Dictionary<string, object?> BuildPlayerState(Player player)
    {
        ...
        if (combatState != null)
        {
            ...
            // Full pile contents (for probability math and decision making)
            state["draw_pile"] = combatState.DrawPile.Cards
                .Select(c => new Dictionary<string, object?> { ["id"] = c.Id.Entry, ["name"] = SafeGetText(() => c.Title), ["cost"] = c.EnergyCost.CostsX ? "X" : c.EnergyCost.GetAmountToSpend().ToString() })
                .ToList();
            state["discard_pile"] = combatState.DiscardPile.Cards
                .Select(c => new Dictionary<string, object?> { ["id"] = c.Id.Entry, ["name"] = SafeGetText(() => c.Title), ["cost"] = c.EnergyCost.CostsX ? "X" : c.EnergyCost.GetAmountToSpend().ToString() })
                .ToList();
            state["exhaust_pile"] = combatState.ExhaustPile.Cards
                .Select(c => new Dictionary<string, object?> { ["id"] = c.Id.Entry, ["name"] = SafeGetText(() => c.Title) })
                .ToList();
    ...
    private static Dictionary<string, object?> BuildMapState(RunState runState)
    {
        ...
        if (player != null)
        {
            ...
            // Deck (full card list for decision making outside combat)
    ...
    private static Dictionary<string, object?> BuildCardSelectState(NCardGridSelectionScreen screen, RunState runState)
    {
        ...
        // Screen type — includes enchant and any other NCardGridSelectionScreen subclasses
        state["screen_type"] = screen.GetType().Name switch
        {
        ...
        // Confirm button — search known containers, then fallback to any NConfirmButton
        ...
        // Fallback: find ANY enabled confirm button in the screen tree (handles enchant, etc.)
        if (!canConfirm)
        {
    ...
    private static Dictionary<string, object?> BuildTreasureState(TreasureRoom treasureRoom, RunState runState)
    {
        ...
        // Report if chest needs to be opened (no auto-open side effect on GET)
        var chestButton = treasureUI.GetNodeOrNull<NClickableControl>("Chest");
        if (chestButton is { IsEnabled: true })
        {
            state["chest_unopened"] = true;
            state["message"] = "Chest is unopened. Use open_chest action to open it.";
            return state;
        }
