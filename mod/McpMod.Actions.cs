...
public static partial class McpMod
{
    private static Dictionary<string, object?> ExecuteAction(string action, Dictionary<string, JsonElement> data)
    {
        ...
        return action switch
        {
            ...
            "open_map" => ExecuteOpenMap(),
            "open_chest" => ExecuteOpenChest(),
            "open_shop" => ExecuteOpenShop(),
    ...
    private static Dictionary<string, object?> ExecuteConfirmSelection()
    {
        ...
        // Check known preview containers first (upgrade uses UpgradeSinglePreviewContainer / UpgradeMultiPreviewContainer,
        ...
        // Try main confirm button by known names
        ...
        // Fallback: find ANY enabled NConfirmButton anywhere in the screen tree.
        // This handles enchant screens (NDeckEnchantSelectScreen) and any future
        // NCardGridSelectionScreen subclasses with non-standard confirm button names.
        ...
        foreach (var btn in allConfirmButtons)
        {
            if (btn.IsEnabled && btn.IsVisibleInTree())
            {
                ...
                {
                    ["status"] = "ok",
                    ["message"] = "Confirming selection (fallback)"
                };
            }
        }
    ...
    private static Dictionary<string, object?> ExecuteDiscardPotion(Player player, Dictionary<string, JsonElement> data)
    {
        ...
        {
            ["status"] = "ok",
            ["message"] = $"Discarding potion '{SafeGetText(() => potion.Title)}' from slot {slot}"
        };
    }
    ...
    private static Dictionary<string, object?> ExecuteOpenChest()
    {
        ...
        if (chestButton is not { IsEnabled: true })
            return Error("Chest is not available or already opened");
    ...
    private static Dictionary<string, object?> ExecuteOpenShop()
    {
        var merchUI = NMerchantRoom.Instance;
        if (merchUI == null)
            return Error("Not in a shop");
    ...
    private static Dictionary<string, object?> ExecuteOpenMap()
    {
        var mapScreen = NMapScreen.Instance;
        if (mapScreen == null)
            return Error("Map screen is not available");
        if (mapScreen.IsOpen)
            return Error("Map screen is already open");
